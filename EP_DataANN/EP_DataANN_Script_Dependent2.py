...
for kk in range(Total_Aggregation_Zone_Number):
    ...
    AggregatedTrain_DF.rename({'Zone_Air_Temperature_':'Zone_Air_Temperature_'+str(kk)}, axis=1, inplace=True)
    AggregatedTest_DF.rename({'Zone_Air_Temperature_': 'Zone_Air_Temperature_' + str(kk)}, axis=1, inplace=True)
    ...
    Train_X = pd.concat(
        [Train_X, ANN_HeatInput_Train_DF[['QAC_Corrected'+str(kk)]].iloc[:-1, :], AggregatedTrain_DF[['Zone_Air_Temperature_'+str(kk)]].iloc[:-1, :]], axis=1)
    ...
    Test_X = pd.concat(
        [Test_X, ANN_HeatInput_Test_DF[['QAC_Corrected'+str(kk)]].iloc[:-1, :],
         AggregatedTest_DF[['Zone_Air_Temperature_'+str(kk)]].iloc[:-1, :]], axis=1)
...
for kk in range(Total_Aggregation_Zone_Number):
    ...
    Predict_Y_Dict1 = {Key_Now : Predict_Y_List}
