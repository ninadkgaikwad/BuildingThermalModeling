...
if (RNN_Type == 1): # Simple RNN
    ...
    Current_ANNModel = tf.keras.Sequential([
        ...
        layers.SimpleRNN(10, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True),
        layers.Dense(1)
    ])
    ...
    Current_ANNModel = tf.keras.Sequential([
        ...
        # layers.Dense(30, activation='relu'),
        # layers.Dense(30, activation='relu', kernel_regularizer='l2'),
        layers.LSTM(10, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True),
        layers.Dense(1)
    ])
    ...
    Current_ANNModel = tf.keras.Sequential([
        ...
        # layers.Dense(30, activation='relu'),
        # layers.Dense(30, activation='relu', kernel_regularizer='l2'),
        layers.GRU(10, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True),
        layers.Dense(1)
    ])
...
for Zone_Number in range(Total_Aggregation_Zone_Number):
    ...
    ANN_PercentageAccuracy_Current_DF = pd.DataFrame([[ANNModel_Key + '_Zone_' + str(Zone_Number1), Train_PercentageAccuracy, Test_PercentageAccuracy, Test_PercentageAccuracy_Sim]],columns=['ANN Model Name', 'Training Accuracy', 'Testing Accuracy Without Simulation','Testing Accuracy With Simulation '])
...
# Saving Data Scaler
TrainDataX_Scaler_FileName = 'TrainDataX_Scaler' + '_Dep1_' + str(Total_Aggregation_Zone_Number) + '.save'
joblib.dump(Scaler_Train_X , os.path.join(Sim_ANNModelData_FolderPath, TrainDataX_Scaler_FileName)) 

TrainDataY_Scaler_FileName = 'TrainDataY_Scaler' + '_Dep1_' + str(Total_Aggregation_Zone_Number) + '.save'
...
Predict_Actual_PHVAC_Dict = {'Predict_Y_BiasAccounted': Predict_Y_List, 'Actual_Y': Actual_Y_List}
