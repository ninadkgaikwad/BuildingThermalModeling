...
AggregatedTest_Dict_File = open(os.path.join(Sim_ProcessedData_FolderPath_AggregatedTestTrain,'Aggregation_DF_Test.pickle'),"rb")
AggregatedTest_DF = pickle.load(AggregatedTest_Dict_File)

AggregatedTrain_Dict_File = open(os.path.join(Sim_ProcessedData_FolderPath_AggregatedTestTrain,'Aggregation_DF_Train.pickle'),"rb")
AggregatedTrain_DF = pickle.load(AggregatedTrain_Dict_File)

# Get Required Files from Sim_RegressionModelData_FolderPath
ANN_HeatInput_Test_DF_File = open(os.path.join(Sim_ProcessedData_FolderPath_Regression,'ANN_HeatInput_Test_DF.pickle'),"rb")
ANN_HeatInput_Test_DF = pickle.load(ANN_HeatInput_Test_DF_File)

ANN_HeatInput_Train_DF_File = open(os.path.join(Sim_ProcessedData_FolderPath_Regression,'ANN_HeatInput_Train_DF.pickle'),"rb")
...
if (
os.path.isdir(os.path.join(Processed_BuildingSim_Data_FolderPath, Simulation_Name, Sim_ANNModelData_FolderName))):
...
for ii in range(ANN_HeatInput_Train_DF.shape[0]):
    QZic_Train_1 = ANN_HeatInput_Train_DF['QZic_P'][ii][0] + ANN_HeatInput_Train_DF['QZic_L'][ii][0] + ANN_HeatInput_Train_DF['QZic_EE'][ii][0]
    QZir_Train_1 = ANN_HeatInput_Train_DF['QZir_P'][ii][0] + ANN_HeatInput_Train_DF['QZir_L'][ii][0] + ANN_HeatInput_Train_DF['QZir_EE'][ii][0] + ANN_HeatInput_Train_DF['QZivr_L'][ii][0]
...
for ii in range(ANN_HeatInput_Test_DF.shape[0]):
    QZic_Test_1 = ANN_HeatInput_Test_DF['QZic_P'][ii][0] + ANN_HeatInput_Test_DF['QZic_L'][ii][0] + ANN_HeatInput_Test_DF['QZic_EE'][ii][0]
    QZir_Test_1 = ANN_HeatInput_Test_DF['QZir_P'][ii][0] + ANN_HeatInput_Test_DF['QZir_L'][ii][0] + ANN_HeatInput_Test_DF['QZir_EE'][ii][0] + ANN_HeatInput_Test_DF['QZivr_L'][ii][0]
...
Train_X = pd.concat([ANN_HeatInput_Train_DF[['QSol1_Corrected','QSol2_Corrected','QZic','QZir','QAC_Corrected']].iloc[:-1,:],AggregatedTrain_DF[['Site_Outdoor_Air_Drybulb_Temperature_','Zone_Air_Temperature_']].iloc[:-1,:]], axis=1)
...
Test_X = pd.concat([ANN_HeatInput_Test_DF[['QSol1_Corrected','QSol2_Corrected','QZic','QZir','QAC_Corrected']].iloc[:-1,:],AggregatedTest_DF[['Site_Outdoor_Air_Drybulb_Temperature_','Zone_Air_Temperature_']].iloc[:-1,:]], axis=1)
...
ANN_PercentageAccuracy_Current_DF = pd.DataFrame([[ANNModel_Key, Train_PercentageAccuracy, Test_PercentageAccuracy]], columns=['ANN Model Name','Training Accuracy','Testing Accuracy'])
...
plt.tight_layout()
plt.savefig(os.path.join(Sim_ANNModelData_FolderPath, ANNModel_Key + '_PairPlot' + '.png'), dpi=300)
# plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(os.path.join(Sim_ANNModelData_FolderPath, ANNModel_Key + '_TrainingPlot' + '.png'))
# plt.show()
...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(Sim_ANNModelData_FolderPath, ANNModel_Key + '_PredictionPlot' + '.png'))
plt.show()
...
ANN_PercentageAccuracy_DF.to_csv(os.path.join(Sim_ANNModelData_FolderPath, "ANN_Model_Accuracy.csv"), index= False)
