...
for kk in range(Total_Aggregation_Zone_Number):
    ...
    Test_Y_Predict_Bias = np.zeros((Test_X_TF_Array_Bias.shape[0]))
    Test_Y_Predict_NoBias = np.zeros((Test_X_TF_Array_Rest.shape[0]))
    Test_Y_Predict = np.zeros((Test_X_TF_Array_Rest.shape[0]))
    # Test_Y_Predict = np.zeros((Test_X_TF_Array.shape[0]))

    # Initializing the Simulation
    PHVAC_Current_Bias = np.zeros((Test_X_TF_Array_Bias.shape[0], 1))
    PHVAC_Current = np.zeros((Test_X_TF_Array_Rest.shape[0], 1))
    PHVAC_Current_NoBias = np.zeros((Test_X_TF_Array_Rest.shape[0], 1))
    ...
    for jj in range(Test_X_TF_Array_Bias.shape[0]):
        ...
        Test_X_TF_Array_Bias[jj,4] = QHVAC_Computed
        Test_X_TF_Array_Bias[jj,6] = Tz_Current
        ...
        PHVAC_Current_Bias[jj,0] = PHVAC_Regression_Model(QHVAC_Computed_Array)
    ...
    Bias = Test_Y_Predict_Bias - Test_Y_Bias

    # Computing the Mean of the Bias
    Bias_Mean = Bias.mean()

    Tz_Current = AggregatedTest_DF['Zone_Air_Temperature_'].iloc[Mid_Index]
    Tz_Current_NoBias = AggregatedTest_DF['Zone_Air_Temperature_'].iloc[Mid_Index]
    Ts = AggregatedTest_DF['System_Node_Temperature_'].to_numpy()[Mid_Index:]
    M_Dot = AggregatedTest_DF['System_Node_Mass_Flow_Rate_'].to_numpy()[Mid_Index:]

    # LOOP: Simulation for Loop with Considering Bias into Account
    for jj in range(Test_X_TF_Array_Rest.shape[0]):

        # Computing QHVAC from Predicted Temperature
        QHVAC_Computed_NoBias = Ca * M_Dot[jj] * (Ts[jj] - Tz_Current_NoBias)
        QHVAC_Computed = Ca * M_Dot[jj] * (Ts[jj] - Tz_Current)

        # Creating X for Current Timestep
        Test_X_TF_Array_Rest[jj,4] = QHVAC_Computed_NoBias
        Test_X_TF_Array_Rest[jj,6] = Tz_Current_NoBias

        # Predicting Next Timestep Temperature
        Test_Y_Predict[jj] = Current_ANNModel(Test_X_TF_Array_Rest[jj,:]) - Bias_Mean
        Test_Y_Predict_NoBias[jj] = Test_Y_Predict[jj] + Bias_Mean

        # Computing PHVAC Current
        QHVAC_Computed_NoBias_Array = np.abs(np.reshape(QHVAC_Computed_NoBias,(1,1)))
        PHVAC_Current_NoBias[jj,0] = PHVAC_Regression_Model(QHVAC_Computed_NoBias_Array)
        QHVAC_Computed_Array = np.abs(np.reshape(QHVAC_Computed,(1,1)))
    ...
    # Computing PHVAC
    PHVAC_NoBias = PHVAC_NoBias + PHVAC_Current_NoBias
    ...
    PHVAC1_Current = PHVAC_Regression_Model.predict(Test_X_Rest['QAC_Corrected'].abs())
    ...
    Test_PercentageAccuracy = (np.absolute((np.mean(Test_Y_Predict1) - np.mean(Test_Y_Rest.to_numpy()))) / np.mean(
        Test_Y_Rest.to_numpy())) * 100

    # Computing Percentage Accuracy of the Model with Simulation
    Test_PercentageAccuracy_Sim = (np.absolute((np.mean(Test_Y_Predict) - np.mean(Test_Y_Rest.to_numpy()))) / np.mean(
        Test_Y_Rest.to_numpy())) * 100

    Test_PercentageAccuracy_Sim_NoBias = (np.absolute((np.mean(Test_Y_Predict_NoBias) - np.mean(Test_Y_Rest.to_numpy()))) / np.mean(
        Test_Y_Rest.to_numpy())) * 100
    ...
    ANN_PercentageAccuracy_Current_DF = pd.DataFrame([[ANNModel_Key, Train_PercentageAccuracy, Test_PercentageAccuracy, Test_PercentageAccuracy_Sim_NoBias, Test_PercentageAccuracy_Sim]],
                                                     columns=['ANN Model Name', 'Training Accuracy', 'Testing Accuracy Without Simulation','Testing Accuracy With Simulation - Bias not accounted', 'Testing Accuracy With Simulation- Bias accounted'])
    ...
    plt.plot(Test_Y_Rest[0:2016], color='b', label='Actual Temp', linestyle='dashed')
    plt.xlabel('Time')
    ...
    plt.figure()
    plt.plot(Test_Y_Predict[0:2016], color='g', label='Predicted Temp accounting Bias')
    plt.plot(Test_Y_Predict_NoBias[0:2016], color='r', label='Predicted Temp without accounting Bias')
    plt.plot(Test_Y_Rest[0:2016], color='b', label='Actual Temp', linestyle='dashed')
    plt.xlabel('Time')
    plt.ylabel('Zone Temperature', labelpad=15)
    plt.legend()
    ...
    Predict_Y_List = np.transpose(np.reshape(Test_Y_Predict, (np.shape(Test_Y_Predict)[0]))).tolist()
    Predict_Y_List_NoBias = np.transpose(np.reshape(Test_Y_Predict_NoBias, (np.shape(Test_Y_Predict_NoBias)[0]))).tolist()
    Actual_Y_List = Test_Y_Rest.tolist()

    Predict_Actual_Y_Dict = {'Predict_Y_BiasNotAccounted': Predict_Y_List_NoBias, 'Predict_Y_BiasAccounted': Predict_Y_List, 'Actual_Y': Actual_Y_List}
    Predict_Actual_Y_DF = pd.DataFrame(Predict_Actual_Y_Dict)

    Predict_Actual_Y_DF_File_Name = 'Predict_Actual_Y_DF_' + str(Total_Aggregation_Zone_Number) + 'Zone_' + str(kk) + '_withSim' + '.pickle'
    ...
    Actual_Y_List1 = Test_Y_Rest.tolist()
    ...
    Predict_Actual_Y_DF_File_Name1 = 'Predict_Actual_Y_DF_' + str(Total_Aggregation_Zone_Number) + 'Zone_' + str(kk) + '_withoutSim' + '.pickle'
